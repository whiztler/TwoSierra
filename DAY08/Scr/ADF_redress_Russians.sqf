...
ADF_fnc_redressRussian_skill = {
	params ["_unit"];
	_unit setskill ['general',(0.6 + random 0.4)];
...
ADF_fnc_redressRussian = {
	...
	_enable_AT		= if ((random 1) > .25) then {true} else {false};
	_clipCount		= 3;
	...
	switch (_priWeapon) do {
		...
		case "arifle_Katiba_GL_F"		: {_clipCount = 7; _unit addPrimaryWeaponItem "optic_Nightstalker"; _unit addPrimaryWeaponItem "muzzle_snds_H"; _unit addBackpack "B_AssaultPack_dgtl"; for "_i" from 1 to 8 do {_unit addMagazine "1Rnd_HE_Grenade_shell"}};
	...
	// Add AT to soldier?
	if (_enable_AT && _priWeapon == "arifle_Katiba_F") then {_unit addBackpack "B_Kitbag_sgg"; [_unit, "launch_RPG32_F", 2, "RPG32_F"] call BIS_fnc_addWeapon;};
