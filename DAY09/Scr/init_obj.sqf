...
_ADF_OpforCntTotal = {(side _x == EAST) && (alive _x)} count allUnits;
...
[] spawn {
	...
	waitUntil {
		private ["_a"];
		sleep 45;		
		_a = nearestObjects [getMarkerPos "mHotel", ["Man"], 50]; 
		({(side _x == WEST) && (alive _x)} count _a) > 0;	
	};
...
[] spawn {
	...
	waitUntil {
		private ["_a"];
		sleep 45;		
		_a = nearestObjects [getMarkerPos "mDelta", ["Man"], 50]; 
		({(side _x == WEST) && (alive _x)} count _a) > 0;	
	};
...
[] spawn {
	...
	waitUntil {
		private ["_a"];
		sleep 45;		
		_a = nearestObjects [getMarkerPos "mRomeo", ["Man"], 30]; 
		({(side _x == WEST) && (alive _x)} count _a) > 0;	
	};
	diag_log "TWO SIERRA: Objective ECHO activated";
	waitUntil {
...
[] spawn {
	waitUntil {
		...
		diag_log format ["TWO SIERRA: Mission win condition CORAZOL - Opfor: %1 | BluFor: %2",_o,_b];
...
[] spawn {
	waitUntil {
		...
		_o = {(side _x == EAST) && (alive _x)} count _a;
		diag_log format ["TWO SIERRA: Mission win condition CALVIN - Opfor: %1",_o];
