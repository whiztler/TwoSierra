...
[] spawn {
	sleep 25;
	while {true} do {
		private ["_c", "_v", "_wp", "_ps", "_pe", "_l"];
		_ps = call ADF_wpPosRdm;
		_pe = call ADF_wpPosRdm;
		_l = [oLand_1,oLand_2] call BIS_fnc_selectRandom;		
		_c = createGroup west;
...
[] spawn {
	...
	while {alive vGunship} do {
		private ["_c", "_u", "_wp", "_p"];	
		_c = createGroup west;
		_c setCombatMode "GREEN";
		_u = _c createUnit ["B_helipilot_F", getMarkerPos "mLMAB",[],0,"LIEUTENANT"]; _u moveInDriver vGunship;
		_u = _c createUnit ["B_helipilot_F", getMarkerPos "mLMAB",[],0,"LIEUTENANT"]; _u moveInGunner vGunship;
		_c setGroupIdGlobal ["6-6 CONDOR"];
		vGunship flyInHeight 50;
		_p = call ADF_wpPosRdm;
		_wp = _c addWaypoint [getMarkerPos _p, 0];
		_wp setWaypointType "MOVE"; _wp setWaypointBehaviour "SAFE"; _wp setWaypointSpeed "NORMAL";
		_p = call ADF_wpPosRdm;		
		_wp = _c addWaypoint [getMarkerPos _p, 0]; _wp setWaypointType "MOVE";
