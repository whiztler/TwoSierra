...
for "_i" from 1 to 5 do {
	private ["_p", "_d", "_v", "_vc", "_patArr"];
	_p	= format ["mGuerVeh_%1",_i];
	_d	= markerDir _p;
	_p 	= getMarkerPos _p;
	_vc	= ["I_G_Offroad_01_armed_F", "I_MRAP_03_hmg_F", "I_MRAP_03_gmg_F", "O_APC_Wheeled_02_rcws_F"] call BIS_fnc_selectRandom;
	...
	if (_vc == "I_G_Offroad_01_armed_F") then {
...
ADF_fnc_BOPactive = {
	...
	_u = _g createUnit ["I_Soldier_F", getPos tBOPspawn, [], 0, "PRIVATE"]; _u moveInGunner sOpfor_01; // Mortar
	_u = _g createUnit ["I_Soldier_F", getPos tBOPspawn, [], 0, "PRIVATE"]; _u moveInGunner sOpfor_02; // HMG small bunker 1 
	_u = _g createUnit ["I_Soldier_F", getPos tBOPspawn, [], 0, "PRIVATE"]; _u moveInGunner sOpfor_03; // HMG small bunker 2
	_u = _g createUnit ["I_Soldier_F", getPos tBOPspawn, [], 0, "PRIVATE"]; _u moveInGunner sOpfor_04; // GMG tower
	_u = _g createUnit ["I_Soldier_F", getPos tBOPspawn, [], 0, "PRIVATE"]; _u moveInGunner sOpfor_05; // HMG tower
	_u = _g createUnit ["I_Soldier_F", getPos tBOPspawn, [], 0, "PRIVATE"]; _u moveInGunner sOpfor_06; // Madrid
	_u = _g createUnit ["I_Soldier_F", getPos tBOPspawn, [], 0, "PRIVATE"]; _u moveInGunner sOpfor_07; // Strider
	{[_x] call ADF_fnc_redressPashtun} forEach units _g;	
...
diag_log	"-----------------------------------------------------";
diag_log format ["TWO SIERRA: AO (excl Opfor Base) spawned. Number of OpFor: %1", {side _x == independent} count allUnits];
diag_log	"-----------------------------------------------------";
